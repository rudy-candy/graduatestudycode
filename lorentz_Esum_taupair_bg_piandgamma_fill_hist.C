...
void lorentz_Esum_taupair_bg_piandgamma_fill_hist()  //(const char* filename)
{
  ...
  std::string filename = Form("lotentz_Esum_taupair_bg_pi and gammma");
  std::string cname = "c_" + filename;
  TCanvas* c = new TCanvas(cname.c_str(), cname.c_str(), 840, 560);
  TH1* h = new TH1F(("h_"+filename).c_str(), ";Energy[GeV]; entry of taupair bg pisum and gamma sum", 300, 0, 15);  
