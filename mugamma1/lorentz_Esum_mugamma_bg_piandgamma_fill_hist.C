...
void lorentz_Esum_mugamma_bg_piandgamma_fill_hist()  //(const char* filename)
{
  ...
  std::string filename = Form("CM_Esum_mugamma pi and gammma");
  std::string cname = "c_" + filename;
  TCanvas* c = new TCanvas(cname.c_str(), cname.c_str(), 840, 560);
  TH1* h = new TH1F(("h_"+filename).c_str(), ";CM Energy[GeV]; entry of mugamma pi sum and gamma sum", 300, 0, 15);  
